<?php
 ...
 // Function to safely fetch the user token
function getToken($db, $userId) {
  $query = "SELECT token FROM login_details WHERE uid = $userId";
  $result = mysqli_query($db, $query);
  
  if ($result && mysqli_num_rows($result) > 0) {
      $row = mysqli_fetch_assoc($result);
      return isset($row['token']) ? $row['token'] : ''; // Avoid undefined index error
  }
  return ''; // Return empty string if no token found
}

// Fetch sender and receiver tokens safely
$senderToken = getToken($db, $senderId);
$receiverToken = getToken($db, $receiverId);

// Insert chat message
$query = "INSERT INTO chat (sender_id, receiver_id, message, is_file, file_name, enc_method, s_token, r_token) 
        VALUES ($senderId, $receiverId, '$eMessage', $isFile, '$fileName', $encMethodId, '$senderToken', '$receiverToken')";

if (!$db->query($query)) {
  echo "Message has not been sent due to an error: " . mysqli_error($db);
}
